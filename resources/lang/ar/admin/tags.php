<?php

return [







    'notActive' => 'غير مفعل ',
    'Active' => ' مفعل ',
    'name' => 'الاسم' ,
    'slug' => '  الاسم بالرابط' ,
    //'photo' => ' الصورة ' ,
    'status' => 'الحالة ' ,
    'photo'=> 'صورة الماركة ' ,
    'opration' =>'الإجراءات ' ,
    'tags' => ' العلامات   ' ,
    'add-tags' => ' أضافة علامة جديدة Tags  ' ,
    'information_tags' => 'بيانات العلامات ' ,
    'tag-name' => 'اسم االعلامة   ' ,
    'all-tags' => 'جميع العلامات  ' ,
    'back' => 'تراجع' ,
    'save' => 'حفظ' ,
    'update' => 'تحديث' ,
    'name.required' => 'يجب إدخال الأسم' ,
    'slug.required' => 'يجب إدخال الرابط بالاسم',
    'message_success' => 'تم التحديث بنجاح ' ,
    'message_error' => 'هناك خطا ما يرجي المحاولة فيما بعد',
    'message_error-not found' => 'هذه العلامة  غير موجودة' ,
    'message_success_add' => 'تمت الاضافة بنجاح ' ,
    'message_delete' => 'تم الحذف بنجاح',




    'edit' => 'نعديل ' ,
    'delete' => 'حذف ' ,
    'edit-tags' => 'تعديل  علامة ' ,
    'information_categories' => 'بيانات  الغلامات   ' ,










];
